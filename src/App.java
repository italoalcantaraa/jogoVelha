...
public class App {
    ...
    public static void jogar(int[] linhaAndColuna, char[][] tabuleiro, char iconePlayer, int modoJogo) {
        ...
        if(modoJogo == 1 || modoJogo == 2 || modoJogo == 3 && iconePlayer == 'X'){
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
        ...
        }else if(modoJogo == 3 && iconePlayer == 'O'){ //Preenche o tabuleiro de acordo com a máquina
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (linhaAndColuna[0] == i && linhaAndColuna[1] == j) {
                        tabuleiro[i][j] = iconePlayer;
                    }
                }
            }    
    ...
    public static boolean verificaVencedor(char[][] tabuleiro, char iconePlayer, int modoJogo) {
        ...
        int permitionAddPoint3 = 0; //Máquina Fácil
        int permitionAddPoint4 = 0;  //Máquina Difícil
        boolean reiniciaPartida = false;

        for (int i = 0; i < 3; i++) {
            if (tabuleiro[i][0] == iconePlayer && tabuleiro[i][1] == iconePlayer && tabuleiro[i][2] == iconePlayer || tabuleiro[0][i] == iconePlayer && tabuleiro[1][i] == iconePlayer && tabuleiro[2][i] == iconePlayer || tabuleiro[0][0] == iconePlayer && tabuleiro[1][1] == iconePlayer && tabuleiro[2][2] == iconePlayer || tabuleiro[0][2] == iconePlayer && tabuleiro[1][1] == iconePlayer && tabuleiro[2][0] == iconePlayer) {
                
                //Nas condições abaixo, verificamos se a reta é preenchida com algum dos ícones dos jogadores 
                ...
                if (iconePlayer == 'X') {
                    ...
                    if(modoJogo == 1){
                        ...
                        System.out.println("Máquina amigável +1 Point");
                        permitionAddPoint3++;
                    }else if(modoJogo == 3){
                        System.out.println("Máquina Inteligente +1 Point");
                        permitionAddPoint4++;
                    }
                    imprimePontuacao(permitionAddPoint1, permitionAddPoint2, permitionAddPoint3, permitionAddPoint4, modoJogo);
                    boolean verificaVencedorGame = verificaVencedorGame(tabuleiro,modoJogo);
    ...
    public static boolean verificaVencedorGame(char[][] tabuleiro, int modoJogo) {
        ...
        if (contadorPoints[0] == 3) {
            ...
            contadorPoints[0] = 0;//Zera a pontuação do player 1
            contadorPoints[2] = 0;//Zera a pontuação da máquina(fácil)
            
            main(null);//chama a main para reiniciar o jogo do zero
            
        }else if(contadorPoints[3] == 3){
            verificaVencedorGame = true;

            System.out.println("** Máquina inteligente ganhou!!! **");
            ...
            contadorPoints[0] = 0;//Zera a pontuação do player 1
            contadorPoints[3] = 0;//Zera a pontuação da máquina(Difícil)
        }
  ...
  public static void MelhorJogada(char[][] tabuleiro, char iconePlayer, int modoJogo) {
    ...
    if (melhorMovimento[0] != -1 && melhorMovimento[1] != -1) {
        ...
        jogadaUsuario(true,melhorMovimento[0],melhorMovimento[1],tabuleiro,iconePlayer,modoJogo);
        ...
        jogadaUsuario(true,linha,coluna,tabuleiro,iconePlayer,modoJogo); //Envia as linhas e colunas
